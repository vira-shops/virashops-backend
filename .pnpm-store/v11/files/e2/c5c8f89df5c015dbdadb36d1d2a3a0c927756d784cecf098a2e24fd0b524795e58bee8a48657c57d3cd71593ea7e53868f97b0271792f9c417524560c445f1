...
exports.renderLogoWithVersion = renderLogoWithVersion;
exports.getDefaultPattern = getDefaultPattern;
...
const check_translations_1 = require("./check-translations");
...
██   ████ ███████ ███████    ██     █████  ███████       ██  ██  █████  ██   ████
`;
function renderLogoWithVersion(logo, version) {
...
function getDefaultPattern(format) {
    return format === 'yaml' ? '*.{yaml,yml}' : '*.json';
}
function getOptionValue(args, index, optionName) {
...
function parseArgs(args) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (!arg.startsWith('-')) {
            throw new Error(`Unexpected positional argument "${arg}". Use flags (for example: -p <dir>).`);
        }
...
function renderHelp() {
    return (`${renderLogoWithVersion(HELP_ASCII_ART, CLI_VERSION)}\n\n` +
...
async function main() {
    ...
    if (options.mode === 'check') {
        ...
        const checkResult = await (0, check_translations_1.checkI18nTranslations)({
            path: options.path,
        ...
        process.stdout.write(`🔎 Found languages: ${checkResult.languages.join(', ') || '(none)'}\n`);
        if (checkResult.ok) {
    ...
    const result = await (0, types_generator_1.generateI18nTypes)({
        path: options.path,
        output: options.out,
        format: selectedFormat,
        filePattern: selectedPattern,
...
//# sourceMappingURL=nestjs-i18n.js.map