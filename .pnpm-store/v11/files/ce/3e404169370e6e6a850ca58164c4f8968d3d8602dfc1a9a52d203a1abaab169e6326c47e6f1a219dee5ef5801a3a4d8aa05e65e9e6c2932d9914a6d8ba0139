type PathKeyOf<T> = T extends any[] ? Exclude<Extract<keyof T, string>, keyof any[]> : Extract<keyof T, string>;
export type Path<T, Depth extends 1[] = []> = [T] extends [Record<string, any>] ? {
    [K in PathKeyOf<T>]: 0 extends 1 & T[K] ? never : [T[K]] extends [Record<string, any>] ? Depth['length'] extends 8 ? K : K | `${K}.${Path<T[K], [...Depth, 1]>}` : K;
}[PathKeyOf<T>] : never;
