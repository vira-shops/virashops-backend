...
import InvalidProductQuestionFieldError from '../../errors/invalid-product-question-field.error';
...
@Injectable()
export default class AnswerProductQuestionUseCase {
  constructor(
  ...
  async execute(
    command: AnswerProductQuestionCommand,
  ): Promise<ProductAnswerProps> {
    const body = command.body.trim();
    if (body.length < 2) {
      throw new InvalidProductQuestionFieldError(
        'Answer body must be at least 2 characters',
      );
    }
    ...
    await this.assertCanAnswer(command, product.getSeller().id);

    return this.questions.saveAnswer({
